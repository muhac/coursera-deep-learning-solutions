...
def neutralize(word, g, word_to_vec_map):
    ...
    e_biascomponent = np.multiply(np.dot(e, g) / np.sum(np.dot(g, g)), g)
...
def equalize(pair, bias_axis, word_to_vec_map):
    ...
    mu_B = np.multiply(np.dot(mu, bias_axis) / np.sum(np.dot(bias_axis, bias_axis)), bias_axis)
    mu_orth = mu - mu_B

    # Step 4: Use equations (7) and (8) to compute e_w1B and e_w2B (≈2 lines)
    e_w1B = np.multiply(np.dot(e_w1, bias_axis) / np.sum(np.dot(bias_axis, bias_axis)), bias_axis)
    e_w2B = np.multiply(np.dot(e_w2, bias_axis) / np.sum(np.dot(bias_axis, bias_axis)), bias_axis)
        
    # Step 5: Adjust the Bias part of e_w1B and e_w2B using the formulas (9) and (10) given above (≈2 lines)
    corrected_e_w1B = np.multiply(np.sqrt(np.abs(1 - np.sum(np.dot(mu_orth, mu_orth)))),
                                 (e_w1B - mu_B) / np.sqrt(np.sum((e_w1 - mu_orth - mu_B) ** 2)))
    corrected_e_w2B = np.multiply(np.sqrt(np.abs(1 - np.sum(np.dot(mu_orth, mu_orth)))),
                                 (e_w2B - mu_B) / np.sqrt(np.sum((e_w2 - mu_orth - mu_B) ** 2)))
