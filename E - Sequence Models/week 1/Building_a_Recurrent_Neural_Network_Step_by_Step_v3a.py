...
def rnn_cell_backward(da_next, cache):
    ...
    dz = 1 - (np.power(a_next, 2))

    # compute the gradient of the loss with respect to Wax (≈2 lines)
    dxt = np.dot(Wax.T, da_next * dz)
    dWax = np.dot(da_next * dz, xt.T)

    # compute the gradient with respect to Waa (≈2 lines)
    da_prev = np.dot(Waa.T, da_next * dz)
    dWaa = np.dot(da_next * dz, a_prev.T)

    # compute the gradient with respect to b (≈1 line)
    dba = np.sum(da_next * dz, axis=1, keepdims=True)
...
def lstm_cell_backward(da_next, dc_next, cache):
    ...
    dWf = np.dot(dft * ft * (1 - ft), concat.T)
    dWi = np.dot(dit * it * (1 - it), concat.T)
    dWc = np.dot(dcct * (1 - np.power(cct, 2)), concat.T)
    dWo = np.dot(dot * ot * (1 - ot), concat.T)
    dbf = np.sum(dft * ft * (1 - ft), axis=1, keepdims=True)
    dbi = np.sum(dit * it * (1 - it), axis=1, keepdims=True)
    dbc = np.sum(dcct * (1 - np.power(cct, 2)), axis=1, keepdims=True)
    ...
    da_prev = (np.dot(parameters['Wf'].T, dft) + np.dot(parameters['Wo'].T, dot) +                np.dot(parameters['Wi'].T, dit) + np.dot(parameters['Wc'].T, dcct))[: n_a, :]
    dc_prev = (da_next * ot * (1 - np.power(np.tanh(c_next), 2)) + dc_next) * ft
    dxt =     (np.dot(parameters['Wf'].T, dft) + np.dot(parameters['Wo'].T, dot) +                np.dot(parameters['Wi'].T, dit) + np.dot(parameters['Wc'].T, dcct))[n_a :, :]
    ### END CODE HERE ###
